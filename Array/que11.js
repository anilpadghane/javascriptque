...
N
 which denotes the number of squares in the chocolate bar.       
...
, consisting of numbers on the chocolate bar squares.        
The third line contains an integer 
D
 and 
M
 which denotes Yatharth's birthday and his birth month respectively.     
Output Format
...
Explanation
Anamika wants to find segments summing to Yatharth's birthday, 
D
=
4
 with a length matching his birth month, 
M
=
2
. In this case, two segments are meeting her criteria: 
[
2
,
2
    ...
    let a = 0;
         for(let i=0; i < arr.length - 1; i++){
           ...
           if(subArray.length === M){
             let sum = subArray.reduce((b,c) => b + c,0);
             if(sum === D){
               a+=1;
             }
           }
